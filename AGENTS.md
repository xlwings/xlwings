...
- Always use fully qualified targets for Python API cross-references, with explicit short display text where appropriate, for example {attr}`Chart.category_axis <xlwings.Chart.category_axis>` or {meth}`set <xlwings.main.ChartAxis.set>`. Match the object's documented namespace (`xlwings` or `xlwings.main`) rather than assuming its implementation module is the correct target. Verify that references render as links in both generated Markdown and HTML; unresolved references can silently render as plain code even when the build succeeds.
