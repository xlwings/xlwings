...
```{note}
The {meth}`view <xlwings.view>` and {meth}`load <xlwings.load>` functions should exclusively be used for interactive work. If you write scripts, use the xlwings API as introduced under [Quickstart](quickstart.md#quickstart) and [Syntax Overview](syntax_overview.md#syntax-overview).
```
