...
- {bdg-info}`Enhancement` xlwings Server and xlwings Lite: Support `typing.Literal` for custom functions args ("enums") to show a dropdown in Excel formulas ({issue}`2695`).
- {bdg-warning}`Bug Fix` xlwings Server and xlwings Lite: Fix type hints for `@script` decorator's `include/exclude` args ({issue}`2696`).
