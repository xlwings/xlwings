...
- All engines: Added `Range.borders` for reading and writing cell borders, see {class}`Borders <xlwings.main.Borders>` and {class}`Border <xlwings.main.Border>`. Line styles, weights and colours are set via lowercase strings (`rng.borders.line_style = "continuous"`) with optional enums `xw.BorderIndex`, `xw.BorderLineStyle` and `xw.BorderWeight` for autocomplete. On xlwings Lite and xlwings Server, reading requires the async API (`await myrange.borders["edge_top"].get_line_style()`), and only the Office.js add-in applies border actions (not Office Scripts, VBA or Google Apps Script). Office.js also doesn't detect a side whose segments differ from cell to cell: where the desktop engines read `None`, an edge reports its first segment's value and an inside border reads `"none"`.
...
- {bdg-info}`Enhancement` Added support for `ndim="natural"`. The key difference with the default behavior is that vertical Excel ranges, i.e., columns, preserve their orientation (`[[1], [2]]`, instead of `[1, 2]`), see [ndim](converters.md#ndim).
...
- {bdg-info}`Enhancement` Added a Polars DataFrame and Series converter, see [Polars DataFrame and Series converters](converters.md#polars-dataframe-and-series-converters) ({issue}`2602`).
...
- {bdg-info}`Enhancement` {bdg-secondary}`PRO` The `@script` decorator now accepts `target_cell` and `config` arguments to bind it to an Office.js sheet button ({issue}`2526`).

## v0.33.0 (Sep 23, 2024)

- {bdg-warning}`Bug Fix` Fixed various issues with VBA compile errors ({issue}`2118` and {issue}`2290`).
- {bdg-info}`Enhancement` {bdg-secondary}`PRO` xlwings Server: deprecate the "DIY" approach in favor of the official [xlwings Server](https://github.com/xlwings/xlwings-server) repo and moved docs to https://server.xlwings.org ({issue}`2519`).
...
- {bdg-success}`Feature` You can now use formatters to format the data you write to Excel or Google Sheets in a very flexible manner (see also [Default Converter](converters.md#default-converter)):
...
- {bdg-success}`Feature` {bdg-secondary}`PRO` Formatters are also available for xlwings Reports via filters: `{{ df | formatter("myformatter") }}`, see [DataFrames Filters](pro/reports/components_filters.md#dataframes-filters).
...
- {bdg-info}`Enhancement` {bdg-secondary}`PRO` The _Remote Interpreter_ has been rebranded to _xlwings Server_.

## v0.27.15 (Sep 16, 2022)

- {bdg-info}`Enhancement` {bdg-secondary}`PRO` Reports: Added new `vmerge` filter to vertically merge cells with the same values, for details, see [vmerge](pro/reports/components_filters.md#vmerge) ({issue}`2020`).
...
- {bdg-success}`Feature` If you still have to write VBA code, you can now use the new CLI command `xlwings vba edit`: this will export all the VBA modules locally so that you can edit them with any editor like e.g., VS Code. Every local change is synced back whenever you save the local file, see [Command Line Client (CLI)](command_line.md#command-line-client-cli) ({issue}`1839`).
...
- {bdg-secondary}`PRO` {bdg-danger}`Breaking Change`: Reports: `create_report()` is now deprecated in favor of `render_template()` that is available via `app`, `book` (new), and `sheet` objects, see: [Quickstart](pro/reports/reports.md#quickstart) ({issue}`1738`).
...
- {bdg-secondary}`PRO` {bdg-info}`Enhancement` {meth}`mybook.to_pdf <xlwings.Book.to_pdf>` now accepts a `layout` parameter so you can "print" your reports onto a PDF with your corporate layout including headers, footers and borderless graphics. See [PDF Layout](pro/reports/components_filters.md#pdf-layout).
...
- {bdg-secondary}`PRO` {bdg-info}`Enhancement`: The reports package now offers the additional DataFrame filters `rowslice` and `colslice`, see [xlwings Reports](pro/reports/reports.md#quickstart) ({issue}`1645`).
- {bdg-secondary}`PRO` {bdg-warning}`Bug Fix`: Bug fix with handling Excel tables without headers.

{bdg-danger}`Breaking Change`

- {bdg-secondary}`PRO` {bdg-info}`Enhancement`: `<frame>` markers now have to be defined as cell notes in the first row, see [Frames: Multi-column Layout](pro/reports/components_filters.md#frames-multi-column-layout). This has the advantage that the Layout view corresponds to the print view ({issue}`1641`). Also, the print area is now preserved even if you use Frames.

v0.24.1 (Jun 27, 2021)

- {bdg-secondary}`PRO` {bdg-info}`Enhancement`: The reports package now offers the additional DataFrame filters `head` and `tail`, see [xlwings Reports](pro/reports/reports.md#quickstart) ({issue}`1633`).
...
- {bdg-secondary}`PRO` {bdg-success}`Feature` Added the `xlwings release` command for an easy release management in connection with the one-click installer, see [1-click Installer/Embedded Code](pro/release.md#1-click-installerembedded-code). ({issue}`1429`).
...
- {bdg-secondary}`PRO` {bdg-success}`Feature`: This release adds support for Markdown-based formatting of text, both in cells as well as in shapes, see [Markdown Formatting](pro/reports/markdown.md#markdown-formatting) for the details. This is also supported for template-based reports.
...
- {bdg-secondary}`PRO` [Docs]: New documentation about how to work with Excel charts in templates; see [Quickstart](pro/reports/reports.md#quickstart).
...
- {bdg-secondary}`PRO` {bdg-success}`Feature`: xlwings Reports now supports template text in shapes, see [xlwings Reports](pro/reports/reports.md#quickstart).
...
- {bdg-info}`Enhancement` Added new method {meth}`Book.to_pdf() <xlwings.Book.to_pdf>` to easily export PDF reports. Needless to say, this integrates very nicely with [xlwings Reports](pro/reports/reports.md#quickstart) ({issue}`1363`).
...
- {bdg-secondary}`PRO` {bdg-info}`Enhancement`: The reports package now supports Excel tables in the templates. This is e.g. helpful to style the tables with striped rows, see [Excel Tables](pro/reports/components_filters.md#excel-tables) ({issue}`1364`).
...
- {bdg-secondary}`PRO` {bdg-success}`Feature`: Added support for [Plotly static charts](matplotlib.md#plotly-static-charts) ({issue}`1309`).
...
- xlwings can now be used with threading, see the docs: [Threading](threading_and_multiprocessing.md#threading) ({issue}`759`).
...
- Config files can now additionally be saved in the directory of the workbooks, overriding the global Ribbon config, see [Making use of Environment Variables](addin.md#making-use-of-environment-variables) ({issue}`772`)
...
- [Mac] Mac Excel 2011 is now supported again with the new add-in. However, since Excel 2011 doesn't support the ribbon,
  the config file has been created/edited manually, see [Making use of Environment Variables](addin.md#making-use-of-environment-variables) ({issue}`714`).
...
5. If you want to use workbook specific settings, add a sheet `xlwings.conf`, see [Workbook Config: xlwings.conf Sheet](addin.md#workbook-config-xlwingsconf-sheet)
