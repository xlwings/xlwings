...
def generate_vba_wrapper(script_vars, f, options):
    ...
    for svar in script_vars.values():
        if hasattr(svar, '__xlfunc__'):
            ...
            with vba.block(func_sig):
                ...
                if TIMING:
                    print_timing = ['Debug.Print "{} {} ";\n'.format(ftype, fname),
                                   'If TypeOf Application.Caller Is Range Then Debug.Print "(from " & Application.Caller.Address & ") ";\n',
                                   'Debug.Print "took : "; format(Timer - time_start,"0.0000"); " seconds"\n' ]
                    for s in print_timing: vba.write(s)
...
def import_udfs(script_path, xl_workbook, options=""):

    script_vars = udf_script(script_path)

    tf = tempfile.NamedTemporaryFile(mode='w', delete=False)
    generate_vba_wrapper(script_vars, tf.file, options)
    tf.close()
