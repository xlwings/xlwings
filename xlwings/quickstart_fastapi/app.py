...
async def validate_api_key(api_key: str = Security(APIKeyHeader(name='Authorization'))):
    """Valide the API_KEY as delivered by the Authorization header
    For production, set a unique API_KEY as environment variable. Without an env var,
    it expects "DEVELOPMENT" as the API_KEY.
    """
